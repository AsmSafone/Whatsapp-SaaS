...
# (`docker exec zaptura npm run cli …`, `npm run export`), and its own bundled dependency tree is
...
RUN if [ "$TARGETARCH" = arm64 ]; then \
        ...
        chown -R zaptura:zaptura /opt/puppeteer && \
...
RUN mkdir -p ./data/sessions ./data/media ./data/plugins && \
    chown -R zaptura:zaptura ./data

# The non-root zaptura user has no home of its own (`useradd -r`, no -m). Chromium resolves the home
# dir from the passwd entry via glib's getpwuid() — it IGNORES $HOME — so it tries to read/write
# /home/zaptura, which does not exist. On hardened/read-only hosts that makes the browser HARD-CRASH
...
# bypassing the passwd lookup); docker-entrypoint.sh creates them owned by zaptura. On a read_only
...
# Copy entrypoint: runs as root to fix named-volume ownership, then drops to zaptura via gosu
...
# then drops to the zaptura user via gosu before starting the node process.
#
# NOTE — no `USER zaptura` directive on purpose (Trivy DS-0002 will flag it, ignore).
# The Node process does NOT run as root: docker-entrypoint.sh:30 is
# `exec gosu zaptura "$@"` after the chowns on lines 7 and 25. Adding `USER zaptura`
# here would run the entrypoint as zaptura and break the chown-before-drop pattern
