 ...
 * `~/.zaptura`, a CI checkout under `~/.cache`, and a `TMPDIR` inside a dotdir all produce it.
...
function makeBuild(root: string): string {
  ...
  writeFileSync(join(root, 'index.html'), '<!doctype html><title>Zaptura Dashboard</title>');
...
const plainDist = makeBuild(join(mkdtempSync(join(dotFreeBase, 'zaptura-dash-plain-')), 'dashboard', 'dist'));
const dottedDist = makeBuild(join(mkdtempSync(join(osTmp, 'zaptura-dash-')), '.dotted', 'dashboard', 'dist'));
...
const RENDER_DISABLED = '/__zaptura_spa_fallback_owned_by_main_ts__';
...
describe.each([
  ...
  it('serves the dashboard index.html at /', async () => {
    ...
    expect(res.text).toContain('Zaptura Dashboard');
  });
  ...
  it('serves built assets with their own content-type (not the SPA shell)', async () => {
    ...
    expect(res.text).not.toContain('Zaptura Dashboard');
  });
