...
const e2ePluginState = join(tmpdir(), `zaptura-e2e-plugins-${e2eRunId}`);
...
for (const [key, name] of [
  ...
  process.env[key] = join(tmpdir(), `zaptura-e2e-${name}-${e2eRunId}`);
}
process.env.DATABASE_TYPE = 'sqlite';
// Isolate the e2e data DB to a throwaway temp file so suites never pollute the developer's
// ./data/zaptura.sqlite. e2e creates sessions/webhooks and doesn't self-clean, so without this they
// pile up across runs. Start each run from a fresh file.
const e2eDataDb = join(tmpdir(), `zaptura-e2e-${process.pid}.sqlite`);
...
const e2eMainDb = join(tmpdir(), `zaptura-e2e-main-${process.pid}.sqlite`);
...
const e2eKeyFile = join(tmpdir(), `zaptura-e2e-key-${process.pid}`);
