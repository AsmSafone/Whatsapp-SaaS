...
export class LoggerService implements NestLoggerService {
  ...
  // (so it lines up with the framework's own output), but tagged honestly as [Zaptura]:
  //   [Zaptura] 1896  - 25/06/2026, 4:59:22 PM     LOG [AuthService] API Docs: … sessionId=abc
  private formatPretty(level: LogLevel, entry: Record<string, unknown>): string {
    ...
    let line =
      `${paint(levelColor, `[Zaptura] ${process.pid}  -`)} ${timestamp} ` +
