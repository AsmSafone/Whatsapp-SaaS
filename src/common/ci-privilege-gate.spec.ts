 ...
 * named-volume ownership and then drops via `exec gosu zaptura`. That drop is the only thing keeping
...
describe('the non-root drop is enforced, not merely documented', () => {
  ...
  it('does not count a commented-out invocation as running the script', () => {
    ...
    expect(executableLines('  ZAPTURA_SMOKE_IMAGE="$IMAGE" ./scripts/smoke-test-non-root.sh # run it')).toContain(
  ...
  it('keeps the entrypoint gosu drop the image depends on', () => {
    ...
    expect(entrypoint).toMatch(/exec\s+gosu\s+zaptura/);
  });
});
