...
describe('docs/10 metric table matches MetricsService', () => {
   ...
   * Deliberately NOT restricted to the `zaptura_` prefix: two series are conventionally unprefixed so
  ...
  /** The table rows: `| \`zaptura_x\` | type | labels | meaning |`. */
  ...
  it('both readers actually read something', () => {
    ...
    expect(emitted().get('zaptura_up')).toBe('gauge');
    expect(documented().size).toBeGreaterThanOrEqual(10);
    expect(documented().get('zaptura_up')).toBe('gauge');
  });
