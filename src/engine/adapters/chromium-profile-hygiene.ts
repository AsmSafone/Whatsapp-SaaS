...
/**
 * SIGKILL any Chromium orphaned by a previous lifetime of this process. When Zaptura dies hard
 ...
 * `--zaptura-session=<id>` marker arg appended to the puppeteer args at launch (Chromium ignores
...
export async function killOrphanedChromiumProcesses(sessionId: string, logger: HygieneLogger): Promise<void> {
  ...
  try {
    ...
    const marker = `--zaptura-session=${sessionId}`;
    ...
    for (const line of psOutput.split('\n')) {
      ...
      // (e.g. a `grep --zaptura-session=…` probing the process table).
