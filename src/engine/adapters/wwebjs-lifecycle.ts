 ...
 * often a persistent browser profile left stale by an Zaptura upgrade that changed the Chromium/Chrome
...
export class WwebjsLifecycle {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // output later (after a hard kill of the Zaptura process orphaned them).
      puppeteerArgs.push(`--zaptura-session=${this.host.config.sessionId}`);
      ...
      if (isExecutionContextDestroyedError(reason)) {
        ...
        this.host.logger.warn(
          `"${reason}" during initialize. If this followed an Zaptura upgrade that changed the ` +
  ...
  private async runInitAttempt(
    ...
    // Kill any Chromium that survived a hard kill of a previous Zaptura process lifetime (its
