...
describe('WhatsAppWebJsAdapter ready reconciliation (#251/#273)', () => {
  ...
  // could not tell an Zaptura self-heal apart from a WhatsApp-side logout or an untouched profile.
...
describe('WhatsAppWebJsAdapter orphaned Chromium sweep (pre-launch)', () => {
  ...
  it('appends the --zaptura-session marker to the puppeteer args handed to the Client', async () => {
    const adapter = newAdapter();

    await adapter.initialize({});

    const client = (adapter as unknown as { client: { options: { puppeteer?: { args?: string[] } } } }).client;
    expect(client.options.puppeteer?.args).toContain(`--zaptura-session=${SESSION_ID}`);
  });
  ...
  it('does not mutate the caller-owned puppeteer args array shared across sessions', async () => {
    ...
    expect(argsB).toContain('--zaptura-session=sess-b');
    ...
    expect(argsB).not.toContain('--zaptura-session=sess-a');
  });
  ...
  it('SIGKILLs a Chromium process carrying this session marker and logs the sweep', async () => {
    mockPsResult({
      stdout: psTable([
        [
          1501,
          `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome --headless --zaptura-session=${SESSION_ID}`,
        ],
  ...
  it('does NOT kill a non-browser process that merely carries the marker string', async () => {
    mockPsResult({
      stdout: psTable([
        [1601, `/bin/grep --zaptura-session=${SESSION_ID}`],
        [1602, `/usr/bin/node scan-sessions.js --zaptura-session=${SESSION_ID}`],
      ]),
    });
  ...
  it('does NOT kill a Chromium process belonging to a different session', async () => {
    mockPsResult({
      stdout: psTable([[1701, '/usr/lib/chromium/chromium --headless --no-sandbox --zaptura-session=session-lain']]),
    });
  ...
  it('does NOT kill a live sibling whose marker merely SHARES A PREFIX with ours (sess vs sess-2)', async () => {
    // `--zaptura-session=sess-orphan` is a substring of `--zaptura-session=sess-orphan-2`: a substring
    // match would SIGKILL the sibling's live browser; the token-exact match must spare it.
    mockPsResult({
      stdout: psTable([
        [1801, `/usr/lib/chromium/chromium --headless --no-sandbox --zaptura-session=${SESSION_ID}-2`],
        [1802, `/usr/lib/chromium/chromium --headless --no-sandbox --zaptura-session=${SESSION_ID}extra`],
      ]),
    });
  ...
  it('kills the orphan when the marker is the LAST token on the command line', async () => {
    mockPsResult({
      stdout: psTable([[1803, `/usr/lib/chromium/chromium --headless --no-sandbox --zaptura-session=${SESSION_ID}`]]),
    });
  ...
  it('runs the orphan sweep before the Singleton cleanup and client.initialize()', async () => {
    ...
    execFileSpy.mockImplementation((...args: unknown[]) => {
      ...
      (args[args.length - 1] as ExecFileCallback)(
        null,
        psTable([[1801, `/usr/bin/chromium --headless --zaptura-session=${SESSION_ID}`]]),
        '',
      );
    });
