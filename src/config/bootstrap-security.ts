...
export interface SecretCheckEnv {
  ...
  /** POSTGRES_BUILTIN — when 'true', Zaptura runs the bundled Postgres on the internal-only network. */
  ...
  s3Endpoint?: string;
  /** MINIO_BUILTIN — when 'true', Zaptura runs the bundled MinIO on the internal-only network. */
  minioBuiltIn?: string;
  apiMasterKey?: string;
...
export function assertNoDefaultSecretsInProduction(env: SecretCheckEnv): void {
  ...
  // 'zaptura'/'minioadmin' credentials are not internet-reachable — exempt them so selecting the
