...
export function validateEnv(config: EnvConfig): EnvConfig {
  ...
  if (dbType === 'postgres') {
    ...
    // PostgreSQL DATABASE_NAME (e.g. 'zaptura') leaking into a SQLite run (#677). That bare name becomes
    // the SQLite file PATH, opening a file named 'zaptura' under the read-only app rootfs →
    ...
    if (dataDbName && !dataDbName.includes('/') && !dataDbName.includes('\\') && !/\.(sqlite|db)$/i.test(dataDbName)) {
      errors.push(
        `DATABASE_NAME must be a file path under the data volume for SQLite (e.g. ./data/zaptura.sqlite); got ${JSON.stringify(
          dataDbName,
        )}. A bare name is the PostgreSQL DB name — leave DATABASE_NAME unset for SQLite to use the default ./data/zaptura.sqlite.`,
      );
    }
  }
