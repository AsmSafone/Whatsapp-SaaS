...
// 0600/0700. That matters because these files ARE secret stores: zaptura.sqlite holds the webhook
...
export class SqlitePermissionsBoot implements OnApplicationBootstrap {
  ...
  onApplicationBootstrap(): void {
    ...
    if (this.config.get<string>('dataDatabase.type', 'sqlite') !== 'postgres') {
      paths.push(this.config.get<string>('dataDatabase.database', './data/zaptura.sqlite'));
    }
