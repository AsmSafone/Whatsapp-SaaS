...
describe('ffmpeg invocation shape', () => {
  const args = buildFfmpegArgs('/tmp/zaptura-convert-x/in.bin', '/tmp/zaptura-convert-x/out.ogg', ['-c:a', 'libopus']);
  ...
  it('names the input immediately after -i, and the output last', () => {
    expect(args[args.indexOf('-i') + 1]).toBe('/tmp/zaptura-convert-x/in.bin');
    expect(args[args.length - 1]).toBe('/tmp/zaptura-convert-x/out.ogg');
  });
