...
describe('DockerService managed specs ↔ docker-compose.yml parity', () => {
  ...
  it('the managed profiles are exactly the compose services labeled as built-in', () => {
    const builtin = Object.entries(compose.services)
      .filter(([, svc]) => (svc.labels ?? []).includes('com.zaptura.builtin=true'))
      .map(([name]) => name)
      .sort();
  ...
  it.each(PROFILES)('%s: attaches to the fixed zaptura-network like the compose service', async profile => {
    const cfg = await capture(profile);
    expect(cfg.HostConfig.NetworkMode).toBe('zaptura-network');
    expect(compose.networks['zaptura-network'].name).toBe('zaptura-network');
    expect(compose.services[profile].networks).toContain('zaptura-network');
    // Compose DNS resolves peers by service name; the Docker-API path adds it as an alias.
    expect(cfg.NetworkingConfig.EndpointsConfig['zaptura-network'].Aliases).toContain(profile);
  });
  ...
  it.each(PROFILES)('%s: binds the same pinned named volume as compose', async profile => {
    ...
    expect(compose.volumes[composeVol].name).toBe(`zaptura_${composeVol}`);
    expect(cfg.HostConfig.Binds).toEqual([`zaptura_${composeVol}:${VOLUME_PATH[profile]}`]);
  });
  ...
  it.each(PROFILES)('%s: sets no CPU/memory/PID limits on either path (only zaptura-api is limited)', async profile => {
  ...
  it('postgres: provisions the fixed built-in credentials; compose defaults agree on user/db only', async () => {
    ...
    expect(cfg.Env).toEqual(['POSTGRES_USER=zaptura', 'POSTGRES_PASSWORD=zaptura', 'POSTGRES_DB=zaptura']);
    ...
    expect(env.POSTGRES_USER).toBe('${DATABASE_USERNAME:-zaptura}');
    expect(env.POSTGRES_DB).toBe('${DATABASE_NAME:-zaptura}');
  ...
  it('postgres: healthcheck resolves to the same pg_isready command as compose', async () => {
    ...
    // Compose interpolates the manual-path user default; the built-in user is always zaptura.
    expect(cfg.Healthcheck!.Test[0]).toBe(composeTest[0]);
    expect(cfg.Healthcheck!.Test[1]).toBe(composeTest[1].replace('${DATABASE_USERNAME:-zaptura}', 'zaptura'));
  });
