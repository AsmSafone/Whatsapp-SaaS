...
export class EventsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleDestroy {
  ...
  private async sweepApiKeyAuthorization(now = Date.now()): Promise<void> {
    ...
    // JWT tenant actors are synthetic (`user:<id>`) and have no api_keys row; treating a miss as
    // "deleted" would drop every dashboard socket on the next sweep.
    const persistedIds = keyIds.filter(id => !id.startsWith('user:'));
    if (persistedIds.length === 0) return;
    const current = await this.authService.findAuthorizationStates(persistedIds);
    const byId = new Map(current.map(key => [key.id, key]));
    for (const keyId of persistedIds) {
