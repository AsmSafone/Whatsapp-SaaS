...
export class InfraConfigController {
  ...
  private assertProductionBootable(merged: Record<string, string>): void {
    ...
    // external switch keeping the bundled 'zaptura' password would save cleanly and crash-loop the
  ...
  async requestRestart(@Body() body?: RestartDto): Promise<{
    ...
    if (this.dockerService.isDockerAvailable()) {
      ...
      // Only ever tear down Zaptura-managed services. An arbitrary profile name (or the empty string)
      ...
      // and drop anything else. (DockerService already hard-prefixes zaptura-<service> and filters on
      // the com.zaptura.service label, so this is defense-in-depth, not the sole control.)
