...
export const INTENTIONALLY_UNEMITTED_ACTIONS: Partial<Record<AuditAction, string>> = {
  ...
  [AuditAction.WEBHOOK_TRIGGERED]:
    'Not emitted: per delivery attempt, redundant with the webhook_delivery_failures dead-letter table and the zaptura_webhook_delivery_failures_total counter.',
  [AuditAction.WEBHOOK_FAILED]:
    'Not emitted: per failed delivery, redundant with the webhook_delivery_failures dead-letter table and the zaptura_webhook_delivery_failures_total counter.',
};
