...
describe('MetricsService', () => {
  ...
  describe('render', () => {
    it('emits Prometheus exposition with session + message gauges', async () => {
      ...
      expect(out).toContain('zaptura_up 1');
      expect(out).toContain('zaptura_sessions_active 2');
      expect(out).toContain('zaptura_sessions_total 3');
      expect(out).toContain('zaptura_sessions{status="ready"} 2');
      expect(out).toContain('zaptura_sessions{status="failed"} 1');
      expect(out).toContain('zaptura_messages_total{direction="outgoing"} 100');
      expect(out).toContain('zaptura_messages_total{direction="incoming"} 50');
      expect(out).toContain('zaptura_messages_failed_total 3');
      // Every metric must declare HELP/TYPE before its sample.
      expect(out).toContain('# TYPE zaptura_messages_total gauge');
      expect(out).toContain('# TYPE zaptura_messages_failed_total gauge');
      // Webhook terminal-failure counter is emitted with correct counter typing + current total.
      expect(out).toContain('# TYPE zaptura_webhook_delivery_failures_total counter');
      expect(out).toContain(`zaptura_webhook_delivery_failures_total ${getWebhookDeliveryFailuresTotal()}`);
      // Reconnect observability counters are emitted with correct counter typing + current totals.
      expect(out).toContain('# TYPE zaptura_session_reconnect_attempts_total counter');
      expect(out).toContain(`zaptura_session_reconnect_attempts_total ${getSessionReconnectAttemptsTotal()}`);
      expect(out).toContain('# TYPE zaptura_session_reconnect_loop_alerts_total counter');
      expect(out).toContain(`zaptura_session_reconnect_loop_alerts_total ${getSessionReconnectLoopAlertsTotal()}`);
      expect(out.endsWith('\n')).toBe(true);
    });
    ...
    it('emits the restricted-session gauge from the live count', async () => {
      ...
      expect(out).toContain('# TYPE zaptura_sessions_restricted gauge');
      expect(out).toContain('zaptura_sessions_restricted 2');
    });
    ...
    it('reports zero restricted sessions rather than omitting the gauge', async () => {
      ...
      expect(out).toContain('zaptura_sessions_restricted 0');
    });
...
describe('MetricsService.render survives a failing stats query', () => {
  ...
  it('still serves the series that need no database', async () => {
    ...
    expect(text).toContain('zaptura_up 1');
    expect(text).toContain('zaptura_process_uptime_seconds');
    expect(text).toContain('zaptura_process_resident_memory_bytes');
    expect(text).toContain('zaptura_webhook_delivery_failures_total');
  });

  it('signals that the database-derived series are missing rather than reporting them as zero', async () => {
    const text = await failing().render();

    expect(text).toContain('zaptura_stats_available 0');
    // A stale or invented 0 would be worse than an absent series: an alert on
    // zaptura_sessions_active would fire as if every session had dropped.
    expect(text).not.toContain('zaptura_sessions_active');
    expect(text).not.toContain('zaptura_messages_total');
  });
  ...
  it('reports the stats source as available on a healthy scrape', async () => {
    ...
    expect(text).toContain('zaptura_stats_available 1');
    expect(text).toContain('zaptura_sessions_active 2');
  });
});
