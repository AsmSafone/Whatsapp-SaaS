...
const response = await fetch('https://zapturawa.com/api/sessions/' + sessionId + '/messages/send-text', {
  method: 'POST',
  headers: {
...
res = requests.post(
    f"https://zapturawa.com/api/sessions/{session_id}/messages/send-text",
...
print("Delivered status:", res.json())`,
  ...
  cURL: {
    label: 'cURL',
    code: `curl -X POST "https://zapturawa.com/api/sessions/$SESSION_ID/messages/send-text" \\
...
func main() {
    url := "https://zapturawa.com/api/sessions/" + os.Getenv("SESSION_ID") + "/messages/send-text"
...
$response = $client->post('https://zapturawa.com/api/sessions/' . $sessionId . '/messages/send-text', [
  'headers' => [
