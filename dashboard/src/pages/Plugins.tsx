...
function PluginConfigUi({ plugin, sessionId }: { plugin: Plugin; sessionId?: string }) {
  ...
  const hardenConfigUiHtml = (source: string): string => {
    const nonce = document.querySelector<HTMLMetaElement>('meta[name="zaptura-csp-nonce"]')?.content ?? '';
    const doc = new DOMParser().parseFromString(source, 'text/html');
    if (nonce && nonce !== '__ZAPTURA_CSP_NONCE__') {
            ...
            <>
              <p className="install-hint">
                {t(
                  ...
                  'Install directly from the Zaptura plugin catalog. The .zip is fetched server-side through the SSRF guard, then validated and sandboxed.',
                )}
              </p>
              {catalogLoading ? (
