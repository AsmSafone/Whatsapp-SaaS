...
# zaptura user via gosu so the Node process never holds root privileges.
set -e

mkdir -p /app/data/sessions /app/data/media /app/data/plugins
chown -R zaptura:zaptura /app/data
...
# Chromium resolves its home from the passwd entry (no /home/zaptura exists), so it hard-crashes at
# launch unless its config/cache dirs exist and are writable. XDG_CONFIG_HOME/XDG_CACHE_HOME (set in
# the image) point here; create them owned by zaptura. On a read_only rootfs these live on tmpfs /tmp,
...
chown zaptura:zaptura "${XDG_CONFIG_HOME:-/tmp/.config}" "${XDG_CACHE_HOME:-/tmp/.cache}"
