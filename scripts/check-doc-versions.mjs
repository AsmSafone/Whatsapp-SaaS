 ...
 *   6. charts/zaptura/Chart.yaml `appVersion` must equal package.json — the chart defaults its image
...
{
  const chart = read('charts/zaptura/Chart.yaml');
  const appVersion = chart.match(/^appVersion:\s*["']?([^"'\s]+)["']?\s*$/m)?.[1];
  if (!appVersion) {
    errors.push('charts/zaptura/Chart.yaml: no appVersion line found — it must name the shipped application version.');
  } else if (appVersion !== version) {
    errors.push(
      `charts/zaptura/Chart.yaml: appVersion is ${appVersion} but package.json is ${version} — the chart would deploy an image tag that does not exist yet.`,
    );
  }
}
