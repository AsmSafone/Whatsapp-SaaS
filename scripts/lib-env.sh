...
# zaptura_env_file_value <file> <key> — print the value from one env-file layer, or nothing.
zaptura_env_file_value() {
...
# zaptura_resolve <key> <default> — the application's precedence: environment, then ./.env, then
...
zaptura_resolve() {
  ...
  for layer in "./.env" "${DATA_DIR:-./data}/.env.generated"; do
    value="$(zaptura_env_file_value "$layer" "$key")"
    if [ -n "$value" ]; then
      printf '%s' "$value"
      return 0
    fi
