...
#   - data store    — zaptura.sqlite (SQLite) OR a pg_dump (when DATABASE_TYPE=postgres)
...
#   - plugin-state/    — registry and persisted ctx.storage state under ZAPTURA_DATA_DIR
#   - .env.generated and .api-key — dashboard config and plaintext bootstrap admin key
#
# The previous runbook backed up the wrong file (zaptura.db) and omitted main.sqlite,
...
#   DATABASE_NAME       data-store SQLite file (default: ./data/zaptura.sqlite; sqlite only)
...
#                       lib-env.sh). They are NOT derived from ZAPTURA_DATA_DIR — the app never does
#                       that either.
#   ZAPTURA_DATA_DIR   data directory for the non-DB state below (default: ./data)
...
# ZAPTURA_DATA_DIR and BACKUP_DIR steer the script itself and are never written to an env file, so
...
DATA_DIR="${ZAPTURA_DATA_DIR:-./data}"
BACKUP_DIR="${BACKUP_DIR:-./backups}"
# shellcheck source=scripts/lib-env.sh
. "$(dirname "$0")/lib-env.sh"
DATABASE_TYPE="$(zaptura_resolve DATABASE_TYPE sqlite)"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

# Database paths resolve exactly like the app: an explicit environment value wins, then ./.env, then
# the dashboard's <data dir>/.env.generated, otherwise the fixed ./data default. ZAPTURA_DATA_DIR
# below only bases the non-DB state directories — deriving DB paths from it would back up files the
# app never reads.
MAIN_DB="$(zaptura_resolve MAIN_DATABASE_NAME ./data/main.sqlite)"
DATA_DB="$(zaptura_resolve DATABASE_NAME ./data/zaptura.sqlite)"
SESSIONS_DIR="$(zaptura_resolve SESSION_DATA_PATH "$DATA_DIR/sessions")"
BAILEYS_DIR="$(zaptura_resolve BAILEYS_AUTH_DIR "$DATA_DIR/baileys")"
MEDIA_DIR="$(zaptura_resolve STORAGE_LOCAL_PATH "$DATA_DIR/media")"
...
PLUGIN_PACKAGES_DIR="$(zaptura_resolve PLUGINS_DIR "$DATA_DIR/plugins")"
...
PLUGIN_STATE_ROOT="$(zaptura_resolve PLUGIN_STATE_DIR "$DATA_DIR")"
...
if [ "$DATABASE_TYPE" = "postgres" ]; then
  ...
  DATABASE_URL_RESOLVED="$(zaptura_resolve DATABASE_URL '')"
  if [ -n "$DATABASE_URL_RESOLVED" ]; then
    ...
    PGPASSWORD="$(zaptura_resolve DATABASE_PASSWORD '')" pg_dump \
      -h "$(zaptura_resolve DATABASE_HOST localhost)" \
      -p "$(zaptura_resolve DATABASE_PORT 5432)" \
      -U "$(zaptura_resolve DATABASE_USERNAME zaptura)" \
      "$(zaptura_resolve DATABASE_NAME zaptura)" >"$STAGE/database.sql"
  fi
  REQUIRED_MEMBERS+=("./database.sql")
else
  log "Backing up data store ($DATA_DB)"
  backup_sqlite "$DATA_DB" "$STAGE/zaptura.sqlite"
  REQUIRED_MEMBERS+=("./zaptura.sqlite")
fi
...
ARCHIVE="$BACKUP_DIR/zaptura-backup-$TIMESTAMP.tar.gz"
