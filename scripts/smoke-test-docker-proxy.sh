#!/bin/sh
# Smoke test: verify zaptura-api can list containers via docker-socket-proxy.
...
echo "==> Checking zaptura-api health..."
...
PROXY_STATE=$(docker inspect --format='{{.State.Status}}' zaptura-docker-proxy 2>/dev/null || echo "not_found")
if [ "$PROXY_STATE" != "running" ]; then
  echo "FAIL: zaptura-docker-proxy is not running (state: $PROXY_STATE)" >&2
  exit 1
fi
echo "PASS: zaptura-docker-proxy is running"

echo ""
echo "==> Verifying the proxy permits the read operations orchestration needs (from zaptura-api)..."
# zaptura-api is the only container that can reach docker-proxy:2375 (internal network), so
...
for endpoint in _ping containers/json; do
  CODE=$(docker exec zaptura-api node -e "fetch('http://docker-proxy:2375/$endpoint').then(r=>console.log(r.status)).catch(()=>console.log(0))" 2>/dev/null || echo 0)
  if [ "$CODE" != "200" ]; then
...
CODE=$(docker exec zaptura-api node -e "fetch('http://docker-proxy:2375/networks').then(r=>console.log(r.status)).catch(()=>console.log(0))" 2>/dev/null || echo 0)
if [ "$CODE" != "403" ]; then
...
# configuration. Zaptura itself never issues deletes (profile teardown is stop-only);
# see SECURITY.md "Docker socket proxy — scope and residual risk".

echo ""
echo "==> Verifying zaptura-api socket mount is gone..."
SOCKET_MOUNT=$(docker inspect zaptura-api --format='{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' 2>/dev/null | grep "docker.sock" || true)
if [ -n "$SOCKET_MOUNT" ]; then
  echo "FAIL: zaptura-api still has a docker.sock mount: $SOCKET_MOUNT" >&2
  exit 1
fi
echo "PASS: zaptura-api has no direct docker.sock mount"
