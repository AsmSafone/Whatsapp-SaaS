...
unset ZAPTURA_DATA_DIR BACKUP_DIR DATABASE_TYPE MAIN_DATABASE_NAME DATABASE_NAME SESSION_DATA_PATH \
...
(
  ...
  MAIN_DATABASE_NAME="$A/custom/auth.sqlite" \
    ...
    ZAPTURA_DATA_DIR="$A/state" \
    BACKUP_DIR="$A/out" \
    "$BACKUP" >/dev/null
)
ARCHIVE_A="$(ls "$A"/out/zaptura-backup-*.tar.gz)"
if ! tar -tzf "$ARCHIVE_A" | grep -qx './main.sqlite'; then
  fail "(a) archive missing ./main.sqlite"
fi
if ! tar -tzf "$ARCHIVE_A" | grep -qx './zaptura.sqlite'; then
  fail "(a) archive missing ./zaptura.sqlite"
fi
(
  cd "$A/restore"
  MAIN_DATABASE_NAME="$A/restore/custom-main.sqlite" \
    DATABASE_NAME="$A/restore/custom-data.sqlite" \
    ZAPTURA_DATA_DIR="$A/restore/state" \
...
mkdir -p "$B"
set +e
OUT_B="$(cd "$B" && ZAPTURA_DATA_DIR="$B/state" BACKUP_DIR="$B/out" "$BACKUP" 2>&1)"
RC_B=$?
set -e
if [ "$RC_B" -eq 0 ]; then
...
# Only the data store missing (default paths) must also fail, naming zaptura.sqlite.
B2="$WORK/b2"
mkdir -p "$B2/data"
...
if ! printf '%s' "$OUT_B2" | grep -q 'zaptura.sqlite'; then
...
if [ "$HAS_SQLITE3" -eq 1 ]; then
  ...
  sqlite3 "$C/src/data/zaptura.sqlite" "CREATE TABLE sentinel(payload TEXT); INSERT INTO sentinel VALUES('c-data');"
  (
    cd "$C/src"
    BACKUP_DIR="$C/out" "$BACKUP" >/dev/null
  )
  ARCHIVE_C="$(ls "$C"/out/zaptura-backup-*.tar.gz)"
  ...
  if [ "$(sqlite3 "$C/dst/data/zaptura.sqlite" 'SELECT payload FROM sentinel;')" != "c-data" ]; then
...
make_fixture "$E/src/data/zaptura.sqlite" "e-data"
if [ "$HAS_SQLITE3" -eq 1 ]; then
  populate_shim "$E/shim" with-sqlite3
else
  populate_shim "$E/shim"
fi
# Shadow tar: create the archive WITHOUT ./zaptura.sqlite to simulate a truncated backup.
...
if [ "\$1" = "-czf" ]; then
  out="\$2"
  shift 2
  exec "$REAL_TAR" -czf "\$out" --exclude='./zaptura.sqlite' "\$@"
fi
exec "$REAL_TAR" "\$@"
EOF
chmod +x "$E/shim/tar"
...
set -e
if [ "$RC_E" -eq 0 ]; then
  fail "(e) min-content check passed an archive missing zaptura.sqlite"
fi
if ! printf '%s' "$OUT_E" | grep -q 'zaptura.sqlite'; then
...
make_fixture "$F/data/zaptura.sqlite" "STALE-data"
printf 'DATABASE_TYPE=sqlite\nMAIN_DATABASE_NAME=%s\nDATABASE_NAME=%s\n' \
  "$F/live/auth.sqlite" "$F/live/store.sqlite" >"$F/state/.env.generated"
(
  cd "$F"
  ZAPTURA_DATA_DIR="$F/state" BACKUP_DIR="$F/out" "$BACKUP" >/dev/null
)
ARCHIVE_F="$(ls "$F"/out/zaptura-backup-*.tar.gz)"
...
if [ "$(db_fingerprint "$F/extract/zaptura.sqlite")" != "foxtrot-live-data" ]; then
...
(
  cd "$F/restore"
  ZAPTURA_DATA_DIR="$F/restore/state" "$RESTORE" "$ARCHIVE_F" >/dev/null
)
...
(
  cd "$F"
  MAIN_DATABASE_NAME="$F/data/main.sqlite" DATABASE_NAME="$F/data/zaptura.sqlite" \
    ZAPTURA_DATA_DIR="$F/state" BACKUP_DIR="$F/out2" "$BACKUP" >/dev/null
)
rm -rf "${F:?}/extract2" && mkdir -p "$F/extract2"
tar -xzf "$(ls "$F"/out2/zaptura-backup-*.tar.gz)" -C "$F/extract2"
...
# $ZAPTURA_DATA_DIR/plugins, so with the knob set the archive carried neither the registry nor any
...
make_fixture "$G/state/zaptura.sqlite" "golf-data"
...
(
  cd "$G"
  ZAPTURA_DATA_DIR="$G/state" PLUGIN_STATE_DIR="$G/elsewhere" BACKUP_DIR="$G/out" \
    MAIN_DATABASE_NAME="$G/state/main.sqlite" DATABASE_NAME="$G/state/zaptura.sqlite" "$BACKUP" >/dev/null
)
ARCHIVE_G="$(ls "$G"/out/zaptura-backup-*.tar.gz)"
...
(
  cd "$G"
  ZAPTURA_DATA_DIR="$G/restore/state" PLUGIN_STATE_DIR="$G/restored-elsewhere" \
    MAIN_DATABASE_NAME="$G/restore/state/main.sqlite" DATABASE_NAME="$G/restore/state/zaptura.sqlite" \
...
make_fixture "$H/src/data/zaptura.sqlite" "hotel-archive-data"
(
  cd "$H/src"
  BACKUP_DIR="$H/out" "$BACKUP" >/dev/null
)
ARCHIVE_H="$(ls "$H"/out/zaptura-backup-*.tar.gz)"
make_fixture "$H/live/main.sqlite" "hotel-live-main"
make_fixture "$H/live/zaptura.sqlite" "hotel-live-data"
set +e
OUT_H="$(cd "$H" && MAIN_DATABASE_NAME="$H/live/main.sqlite" \
  DATABASE_NAME="$H/live/zaptura.sqlite" ZAPTURA_DATA_DIR="$H/live" \
...
if [ "$(db_fingerprint "$H/live/zaptura.sqlite")" != "hotel-live-data" ]; then
...
(
  cd "$H"
  MAIN_DATABASE_NAME="$H/live/main.sqlite" DATABASE_NAME="$H/live/zaptura.sqlite" \
    ZAPTURA_DATA_DIR="$H/live" "$RESTORE" "$ARCHIVE_H" --force >/dev/null
)
...
if [ "$(db_fingerprint "$H/live/zaptura.sqlite")" != "hotel-archive-data" ]; then
...
make_fixture "$I/live/zaptura.sqlite" "india-live-data"

# guarded_restore <target dir>: restore ARCHIVE_H without --force over <dir>/main.sqlite and
# <dir>/zaptura.sqlite, with $I/bin first on PATH. Output lands in OUT, the exit code in RC.
guarded_restore() {
  set +e
  OUT="$(cd "$WORK" && PATH="$I/bin:$PATH" MAIN_DATABASE_NAME="$1/main.sqlite" \
    DATABASE_NAME="$1/zaptura.sqlite" ZAPTURA_DATA_DIR="$1" "$RESTORE" "$ARCHIVE_H" 2>&1)"
  RC=$?
  set -e
}
...
expect_refused() {
  ...
  if ! printf '%s' "$OUT" | grep -qF "$I/live/zaptura.sqlite"; then
    fail "($1) refusal message does not name the live data store"
  fi
  if [ "$(db_fingerprint "$I/live/zaptura.sqlite")" != "india-live-data" ]; then
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  make_fixture "$L/data/zaptura.sqlite" "l-data"
  chmod a-w "$L/ro"
  set +e
