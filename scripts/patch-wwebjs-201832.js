 ...
 * name). whatsapp-web.js 1.34.7 (what Zaptura pins) reads `_serialized` in the
 ...
 * `Base._normalizeId` (i.e. upstream shipped #201832 and Zaptura bumped its dep).
 *
 ...
 *
 * GIT_CEILING_DIRECTORIES is what makes it work at all: without it git discovers the parent Zaptura
...
function applyBackport(wwjsDir = DEFAULT_WWJS, patchFile = DEFAULT_PATCH) {
  ...
  // helper: Message.js is the load-bearing site (Zaptura reads `msg.id._serialized` in ~40 places),
