...
#   DATABASE_NAME       restore target for the SQLite data store (default: ./data/zaptura.sqlite)
...
#                       derived from ZAPTURA_DATA_DIR — restoring there would write databases the
#                       app never reads (fresh-empty boot + new master key).
#   ZAPTURA_DATA_DIR   data directory to restore non-DB state into (default: ./data)
#   SESSION_DATA_PATH, BAILEYS_AUTH_DIR, STORAGE_LOCAL_PATH, PLUGINS_DIR
#                     override the corresponding state directories
#
# Stop the Zaptura app before restoring. A snapshot of the current data dir is taken
...
# outside ZAPTURA_DATA_DIR. This reads the config of the install being restored INTO, which is why it
...
RESOLVED_USER_HOME="$(resolve_path "${HOME:-/nonexistent-zaptura-home}")"

log() { echo "[restore] $*"; }

case "$DATA_DIR" in
  '' | / | . | ./ | .. | ../)
    log "ERROR: refusing unsafe ZAPTURA_DATA_DIR target: ${DATA_DIR:-<empty>}"
    exit 1
    ;;
esac
case "$RESOLVED_CWD/" in
  "$RESOLVED_DATA_DIR"/*)
    log "ERROR: ZAPTURA_DATA_DIR must not be the workspace or one of its parent directories: $DATA_DIR"
    exit 1
    ;;
esac
if [ "$RESOLVED_DATA_DIR" = "$RESOLVED_USER_HOME" ]; then
  log "ERROR: ZAPTURA_DATA_DIR must not be the user home directory: $DATA_DIR"
  exit 1
fi
...
if [ "$FORCE" -ne 1 ]; then
  ...
  if [ -f "$STAGE/zaptura.sqlite" ] && db_appears_live "$DATA_DB"; then
