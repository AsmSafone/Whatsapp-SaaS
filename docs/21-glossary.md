...
An interface implementation that provides a specific capability. In Zaptura, adapters are used for:
...
Authentication token to access the Zaptura API. Sent via the `X-API-Key` header.
...
Node.js library for WhatsApp Web that uses WebSocket directly without a browser (no Chromium required). Available as a selectable engine in Zaptura via `ENGINE_TYPE=baileys`.
...
A Redis-based Node.js job queue library (`bullmq`, wired in through `@nestjs/bullmq`). It backs the two queues Zaptura runs:
...
Browser used by Puppeteer to run WhatsApp Web. Zaptura uses headless Chromium.
...
Web interface to manage Zaptura without using the API directly. Built with React and Vite, using TanStack Query for data fetching and plain CSS for styling.

### Dead Letter Queue (DLQ)

The durable record of deliveries abandoned after every retry. In Zaptura it is a **database table**, not a Redis queue: `webhook_delivery_failures` for outbound webhooks and `integration_delivery_failures` for plugin ingress. Used for debugging; **redrive exists for plugin ingress only** (`POST /api/integration/instances/:pluginId/:instanceId/redrive`); an outbound-webhook failure row is a record, not a replay source, so recovery means the provider or your own tooling re-sending the event.

### Docker

Containerization platform for packaging and deploying applications. Zaptura is distributed as a Docker image.

## E

### Engine

Component that handles communication with WhatsApp Web. Zaptura supports pluggable engines selected via the `ENGINE_TYPE` environment variable: `whatsapp-web.js` (default, Chromium/Puppeteer-based) or `baileys` (browser-free, WebSocket-based).
...
GitHub Container Registry - registry for storing Docker images. The Zaptura image is available at `ghcr.io/asmsafone/zaptura` (and, mirrored per release, at `docker.io/asmsafone/zaptura`).
...
WhatsApp's id format, inherited from XMPP; the user-facing "Chat ID" is a JID. The same entity can be addressed in more than one dialect: `<phone>@c.us` (whatsapp-web.js, and Zaptura's neutral form), `<phone>@s.whatsapp.net` (Baileys' raw form for the same user), `<id>@g.us` (a group), or `<lid>@lid` (a LID, a privacy id). WhatsApp also issues Meta-hosted dialects of the first and last of these (`@hosted`, `@hosted.lid`); they name the same account and normalize to the same neutral form. Zaptura normalizes engine ids to a single neutral dialect at the engine boundary - see _System Architecture > WhatsApp Identity Contract_.

### Job Queue

Queueing system for asynchronous task processing. Zaptura registers exactly two queues — `webhook-queue` and `ingress-queue` — both optional (`QUEUE_ENABLED`). There is no scheduled or delayed sending: outbound messages are dispatched inline by the request that asks for them.

## L

### LID (Linked ID)

A WhatsApp **privacy identifier** (`<number>@lid`) that addresses a user without exposing their phone number - increasingly used in groups and communities. Its number is **not** a phone number; a separate `lid -> phone` mapping (supplied by WhatsApp via history sync / contacts) resolves it when known. Zaptura keeps an unresolved LID as-is rather than guessing a phone. See _System Architecture > WhatsApp Identity Contract_.
...
Not a component of Zaptura. Outbound sends are synchronous; the only queues are `webhook-queue` (outbound webhook delivery) and `ingress-queue` (inbound plugin events) — see _Job Queue_. Bulk sending paces itself with a per-message delay rather than a queue.
...
Ability to run multiple WhatsApp sessions within a single Zaptura instance.

## N

### NestJS

Node.js framework for building server-side applications. The Zaptura backend is built with NestJS.

### Node.js

JavaScript runtime used to run Zaptura. Recommended version: Node.js 22 LTS.

## O

### ORM (Object-Relational Mapping)

Library that maps objects in code to database tables. Zaptura uses TypeORM.
...
Extension that can be added to Zaptura to add functionality without modifying the core codebase.
...
Architectural style for APIs used by Zaptura. Uses HTTP methods (GET, POST, PUT, DELETE).
...
AWS object storage service. Zaptura supports S3-compatible storage for media files.
...
API documentation tool. Zaptura provides Swagger UI at `/api/docs`.
...
Typed superset of JavaScript used for Zaptura development.
...
Persistent storage for Docker containers. Zaptura data is stored in volumes.

## W

### WAHA

WhatsApp HTTP API - a similar project that inspired Zaptura. Zaptura is built as an open-source alternative.
...
Default engine library used by Zaptura to interact with WhatsApp Web. Uses Puppeteer to control a headless Chromium browser. Selected via `ENGINE_TYPE=whatsapp-web.js` (or by omitting the env var).
