...
Zaptura runs **untrusted plugins** (anything loaded from the plugins directory) in an isolated worker
...
as the same uid as Zaptura. The sandbox therefore does **not**, by itself, stop a malicious plugin
from reading files the Zaptura process can read or making outbound network connections. It protects
...
- **Run Zaptura in a container.** The image's entrypoint already drops to the non-root `zaptura` user
