...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   n8n Workflow  │────▶│  Zaptura Node    │────▶│  Zaptura API     │
│                 │     │  (credentials)  │     │  (your server)  │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                        │
                                                        ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   n8n Workflow  │◀────│ Zaptura Trigger  │◀────│  Webhook POST   │
│   (triggered)   │     │  (listens)      │     │  from Zaptura    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```
...
2. Select **Install**
3. Enter `@asmsafone/n8n-nodes-zaptura`
...
```bash
cd ~/.n8n/nodes
npm install @asmsafone/n8n-nodes-zaptura
```
...
Execute operations on your Zaptura server.

#### Credentials Setup

| Field      | Description                      | Example                  |
| ---------- | -------------------------------- | ------------------------ |
| Server URL | Zaptura server URL (without /api) | `https://wa.example.com` |
| API Key    | API key from Zaptura dashboard    | `zap_xxxxxxxx...`        |
...
1. When workflow is activated, the trigger creates a webhook in Zaptura
2. Zaptura sends events to n8n's webhook URL
...
> `X-Zaptura-Idempotency-Key` / `X-Zaptura-Delivery-Id` headers. `idempotencyKey` is **stable across retries**
> of the same event; `deliveryId` identifies one delivery to one webhook and is stable across that
> delivery's retry attempts too — read the `X-Zaptura-Retry-Count` header for the attempt number. Because a
...
```
[Zaptura Trigger] → [IF: Check keyword] → [Zaptura: Send Text]
     │
...
- Zaptura: Send Text with welcome message
...
```
[Zaptura Trigger] → [Google Sheets: Append] → [Zaptura: Send Text]
     │                    │
...
```
[Zaptura Trigger] → [Slack: Send Message]
     │
...
```
[Webhook: New Order] → [Zaptura: Send Text]
                            │
...
```
[Schedule Trigger] → [Google Sheets: Get Rows] → [Loop] → [Zaptura: Send Text]
     │                      │                                    │
...
```
[Zaptura Trigger] → [IF: Booking intent?] → [Set: Normalize request]
                                               │
                                               ▼
                                      [Availability Source]
                                               │
                         ┌─────────────────────┴─────────────────────┐
                         ▼                                           ▼
              [Create Booking] → [Zaptura: Send Text]      [Zaptura: Send Text]
...
```
[Zaptura Node] → [IF: Check success] → [Continue...]
                      │
                      └── [Error Handler]
```
...
```
[Loop Over Items] → [Wait: 2 seconds] → [Zaptura: Send Text]
```
...
1. Verify Zaptura server is running
2. Check API key is correct
3. Ensure server URL doesn't have trailing slash
4. Verify network connectivity between n8n and Zaptura
...
1. **Confirm you registered the production webhook URL, not the test one.** n8n gives every Webhook
   ...
   Activate the workflow and point Zaptura at the production URL.
2. Check webhook was created in Zaptura dashboard
3. Verify n8n webhook URL is accessible from Zaptura server
...
6. For a call trigger, check the session's engine: the call-outcome events never fire on
   ...
   table above)
7. Ask Zaptura which side dropped the event:
   `GET /api/webhooks/delivery-failures?sessionId={sessionId}` (ADMIN key). A row means Zaptura
...
- [Zaptura API Specification](./06-api-specification.md)
