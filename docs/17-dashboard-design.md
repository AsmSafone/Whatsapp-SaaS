...
The dashboard is a web-based management interface for Zaptura that lets users manage sessions, webhooks, and monitor activity without using the API directly.
...
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  🔗 Main Webhook                                   ✅ Active   │  │
│  │  https://api.example.com/webhook/zaptura                        │  │
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const apiKey = sessionStorage.getItem('zaptura_api_key');
...
`localStorage` under `zaptura_theme`:
...
maintainability; the legacy `zaptura_palette` storage key and the attribute are cleaned up on load.
...
export function useTheme() {
  const [theme, setTheme] = useState<Theme>(/* localStorage 'zaptura_theme' ?? 'system' */);

  useEffect(() => {
    const root = document.documentElement;
    if (theme === 'system') root.removeAttribute('data-theme');
    else root.setAttribute('data-theme', theme);
    localStorage.setItem('zaptura_theme', theme);
  }, [theme]);
