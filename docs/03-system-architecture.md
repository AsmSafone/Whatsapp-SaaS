...
flowchart TB
    ...
    subgraph Zaptura["Zaptura Platform"]
...
Zaptura is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to larger single-node, multi-session installs.

> **Note — single-instance:** the live WhatsApp engine layer is stateful and held in-process
> (an in-memory `Map` in `EngineRegistry`). Zaptura currently runs as **one API instance per
...
Zaptura does **not** use a dynamic `AdaptersModule` or string DI tokens. `AppModule`
...
flowchart TB
    subgraph Production["Production Environment"]
        API[Zaptura API<br/>single instance]
...
> session registry, node claim, or affinity router in the codebase. Zaptura runs as one API instance
...
> [!IMPORTANT]
> Engine abstraction is critical to mitigate **R001: WhatsApp Protocol Changes** in Risk Management. Zaptura ships two production-ready engines selectable via `ENGINE_TYPE`: `whatsapp-web.js` (default, Chromium/Puppeteer-based) and `baileys` (browser-free, WebSocket/Noise protocol). With the abstraction layer, adding further engines requires no changes to application code.
...
Zaptura uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run Zaptura without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["Zaptura Core"]
...
Zaptura supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
if (dbType === 'postgres') {
  return {
    ...
    database: configService.get('dataDatabase.name', 'zaptura'),
...
return {
  ...
  database: configService.get('dataDatabase.database', './data/zaptura.sqlite'),
  synchronize,
...
> **Note:** Zaptura does not currently apply SQLite-specific concurrency hardening. There is **no**
...
Zaptura provides several deployment profiles for different needs:
...
> Zaptura runs as a single API instance per session-data volume; there is no cluster-mode flag.
...
Zaptura does not auto-detect a profile at runtime; pick one by available resources and expected load:
