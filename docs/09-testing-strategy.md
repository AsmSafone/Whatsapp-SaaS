...
Zaptura now has an active Jest test suite covering the backend core, engine adapters, security helpers,
...
| `docker`        | multi-arch Docker build on pushes and pull requests, then `scripts/smoke-test-non-root.sh` against the built image so the entrypoint's root→zaptura drop is verified, not assumed; publishes to GHCR only on push, so fork pull requests validate both architectures without publishing                                                                                                                                                                                                |
...
docker compose logs -f zaptura-api
```
