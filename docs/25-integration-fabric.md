...
The overriding goal is to preserve the untrusted-worker safety invariants _by construction_. Zaptura
...
**~90% a faithful clone of seams Zaptura already ships**:
...
flowchart LR
    ...
    subgraph Core["Zaptura host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an Zaptura API key, so ingress is public to
...
[Zaptura-plugins](https://github.com/AsmSafone/Whatsapp-SaaS-plugins) catalog, not in this repository. What
