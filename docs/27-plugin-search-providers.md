...
(Add the Zaptura repo as a devDependency or reference the types via a `paths` mapping. A standalone
`@zaptura/plugin-types` package is planned.)
...
`registerWebhook`, and `registerSearchProvider`. (A formal `@zaptura/plugin-types` package with the full
