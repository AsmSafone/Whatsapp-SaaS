...
Zaptura supports multiple infrastructure configurations. Choose based on your needs:
...
# Optional: seed a known admin key. If omitted, Zaptura generates a random key and writes data/.api-key.
API_MASTER_KEY=
...
# View Docker logs (service is `zaptura-api` in docker-compose.yml, `zaptura` in
...
**Check logs:**
```bash
docker compose logs zaptura-api --tail 100
````
...
docker run --shm-size=2gb zaptura
```
