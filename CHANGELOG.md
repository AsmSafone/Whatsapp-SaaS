...
- The upgrade runbook and the migration guide state the `docker-compose.dev.yml` caveat before the first
  `docker compose` command instead of after it, and name the `zaptura` service substitution it needs.
...
- The `zaptura_sessions_restricted` gauge now follows restrictions that lapse on their own instead of reporting the pre-expiry count indefinitely.
...
- `GET /api/metrics` no longer `500`s when the data database is unreachable; database-derived series are omitted rather than zeroed, and a new `zaptura_stats_available` gauge says which happened. The metrics reference now lists every emitted series, gated against the renderer.
...
- The published image's drop to the `zaptura` user is now verified in CI — the smoke test previously ran in no workflow.
...
- `asmsafone-zaptura` 0.2.0 on PyPI, the first release through the trusted-publishing workflow.
- `asmsafone/zaptura` 0.2.0 on Packagist, the first versioned PHP release since June; Composer users on a stable constraint were pinned to 0.1.0.
...
- The JavaScript SDK publishes to npm from CI via Trusted Publishing (OIDC) on a `js-sdk-v*` tag — no npm token exists anywhere, and every release carries build provenance. First release: `@asmsafone/zaptura@0.2.0`.
...
- Helm chart for Kubernetes deployments under `charts/zaptura/`. Closes #695.
...
- `scripts/zaptura.sh`, an orchestration helper superseded by the in-process Docker orchestration on `/api/infra`.
...
- Orphan-Chromium sweep now matches the `--zaptura-session=<id>` marker token-exactly, so restarting `sales` no longer kills sibling `sales2`. (#923)
...
- Completed `.dockerignore` (`.git/`, `dashboard/node_modules`, `*.sqlite`, agent workspaces, etc.) with a `check-dockerignore.mjs` CI gate, and extracted `postinstall` to `scripts/postinstall.js` with failure propagation. **Breaking (behavior):** `docker pull zaptura:latest` no longer moves on a main merge; `npm install` fails on a broken dashboard/patch. (#943)
...
- Release images are now dual-published bit-identical to `docker.io/asmsafone/zaptura` alongside GHCR, with provenance and SBOM attestations verified pullable before release.
...
- The README gains a **"Before you connect a number"** section: Zaptura is unofficial, a per-engine ban-risk vs. resource-cost table, six safe-sending guardrails, the known cold-contact first-send silent drop (#830), and a pointer to the official Cloud API. Responds to discussions #87, #154, #436, #687, #694.
...
- The linked-device name is now brandable via the optional `BAILEYS_BROWSER_NAME` env var (default `Zaptura`). Thanks @clicsoluciones. (#822)
...
- Reconnect-loop observability: an `zaptura_session_reconnect_attempts_total` counter, plus a `session.reconnect_loop` webhook, warning log, and `zaptura_session_reconnect_loop_alerts_total` tick on every fifth consecutive attempt; the streak re-arms after a stable connection.
- The whatsapp-web.js engine sweeps orphaned Chromium processes carrying an `--zaptura-session=<id>` marker before each (re)launch.
...
- Prometheus counter `zaptura_webhook_delivery_failures_total` on `/api/metrics`, incremented once per delivery that exhausts its retries.
...
- Bundled Compose no longer kills Chromium mid-spawn under multi-session whatsapp-web.js load; `pids_limit` default raised 512 → 2048, exposed as `ZAPTURA_PIDS_LIMIT` (#636).
...
- Official Java SDK (`com.asmsafone:zaptura`): a synchronous Java 17 client covering all 12 REST resources plus API-key validation, published to Maven Central as `com.asmsafone:zaptura:0.1.1`. (#602)
...
- Send true WhatsApp voice notes (PTT): `send-audio`, bulk send, and the `MessageSendAudio` tool accept an optional `ptt`; the server defaults the mimetype to `audio/ogg; codecs=opus` and stores `type: "voice"`. (Zaptura-n8n #13)
...
- Docker container teardown on `POST /infra/restart` is restricted to the managed allowlist (`postgres`/`redis`/`minio`) with exact `zaptura-<service>` matching. (#534)
...
- Client SDKs: official hand-written libraries for JavaScript/TypeScript (`@asmsafone/zaptura`), Python (`asmsafone-zaptura`), and PHP (`asmsafone/zaptura`), each with the same fluent resource surface, a typed error hierarchy, and server-mirroring types; published at `0.1.0`. (#463)
...
- `dashboard/nginx.conf` now targets `zaptura-api` for `/api/` and `/socket.io/`. Thanks @Abhishekrajpurohit (#259).
...
- Headless Chromium now starts as the non-root `zaptura` user in the Docker image. (closes #242)
...
- Datastore secrets are now required — no `zaptura`/`minioadmin` default; production refuses to boot with placeholders.
- Bull Board `?apiKey=` removed — authenticate via `X-API-Key`/`Authorization: Bearer`.
- New env knobs: `SSRF_ALLOWED_HOSTS`, `MEDIA_DOWNLOAD_MAX_BYTES`, `MEDIA_DOWNLOAD_TIMEOUT_MS`, `MAIN_DATABASE_SYNCHRONIZE`, `SHUTDOWN_DELAY_MS`, `ZAPTURA_MEM_LIMIT`, `METRICS_TOKEN`.
...
- Docker: the container reaches the Docker API via a least-privilege `docker-socket-proxy` over TCP; Node runs as non-root `zaptura` via a `gosu` entrypoint (`dumb-init` PID 1). Thanks @A831ARD0 (#227, #228; supersedes #129).
...
First stable release of the Zaptura WhatsApp API Gateway.
