...
Zaptura is a self-hosted WhatsApp API gateway. It handles API-key authentication,
...
Zaptura already ships several hardening measures: API-key auth with roles
(ADMIN / USER), optional outbound webhook SSRF protection, a production
...
When exposing Zaptura, please review the security-relevant configuration documented in
...
Installing or enabling a plugin is executing third-party code on the host that runs Zaptura.
...
Zaptura process — run them in a separate container/VM with an OS-level sandbox and reach
Zaptura over the API like any other client.
...
Mitigations in place: the proxy is unreachable except from `zaptura-api` (dedicated
...
and Zaptura itself never issues deletes (profile teardown is stop-only). If you do not
