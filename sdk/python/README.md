...
Requires Python 3.9+. The importable module is `zaptura`.
...
```python
with ZapturaClient(base_url="…", api_key="…") as client:
...
```python
import httpx
client = ZapturaClient(base_url="…", api_key="…", transport=httpx.MockTransport(handler))
```
...
A non-2xx response raises a typed `ZapturaApiError` subclass — `ZapturaAuthError` (401),
`ZapturaForbiddenError` (403), `ZapturaNotFoundError` (404), `ZapturaConflictError` (409),
`ZapturaRateLimitError` (429), `ZapturaNotImplementedError` (501),
`ZapturaServiceUnavailableError` (503 — the only retryable one) — each carrying `.status`
and the parsed `.body`. A timeout raises `ZapturaTimeoutError`. In a routed deployment only
...
```python
from zaptura import ZapturaNotFoundError

try:
    client.sessions.get("missing")
except ZapturaNotFoundError as e:
    print(e.status)  # 404
```
