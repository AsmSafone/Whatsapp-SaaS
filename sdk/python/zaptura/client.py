"""Zaptura Python SDK — client core.

The :class:`ZapturaClient` is the single entry point. It owns an
...
def _warn_if_insecure_http(url: str) -> None:
    ...
    try:
        ...
        if parsed.scheme == "http" and host not in _LOCALHOST_HOSTS:
            warnings.warn(
                f"ZapturaClient: base_url uses an insecure http:// URL (host: {host}). "
...
class ZapturaClient:
    def __init__(
        ...
        *,
        ...
        if not base_url:
            raise ValueError("ZapturaClient: base_url is required")
        if not api_key:
            raise ValueError("ZapturaClient: api_key is required")
    ...
    def __enter__(self) -> "ZapturaClient":
        return self
