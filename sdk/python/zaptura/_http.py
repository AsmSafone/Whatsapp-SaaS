"""HTTP transport wrapper for the Zaptura Python SDK.
...
from .errors import ZapturaApiError, ZapturaTimeoutError, classify
...
class HttpExecutor:
    ...
    Constructed once per :class:`ZapturaClient`; the transport is taken from
    ...
    def _send(self, method: HttpMethod, path: str, *, query: Mapping[str, Any] | None, body: Any) -> httpx.Response:
        ...
        try:
            ...
            raise ZapturaTimeoutError(self._timeout) from e
        ...
        if res.status_code >= 300:
            ...
            raise ZapturaApiError.from_response(res.status_code, res.text, context)
        return res
