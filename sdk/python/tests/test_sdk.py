"""Unit tests for the Zaptura Python SDK — assert exact paths and bodies."""
...
from zaptura import ZapturaClient, ZapturaApiError, ZapturaNotFoundError
from zaptura._http import build_url
from zaptura.errors import ZapturaServiceUnavailableError
from zaptura.types import WebhookFilters
...
class TestClientCore:
    def test_requires_base_url_and_api_key(self):
        with pytest.raises(ValueError):
            ZapturaClient(base_url="", api_key="k")
        with pytest.raises(ValueError):
            ZapturaClient(base_url="http://x", api_key="")
    ...
    def test_non_json_2xx_body_returns_text(self):
        ...
        client = ZapturaClient(base_url="http://x", api_key="k", transport=httpx.MockTransport(handler))
    ...
    def test_treats_unfollowed_redirect_as_error(self):
        ...
        with pytest.raises(ZapturaApiError):
            client.sessions.list()
        assert len(calls) == 1  # the redirect target was never requested

    def test_strips_trailing_slash(self):
        backend = MockBackend().on("GET", "/api/sessions", body=[])
        client = ZapturaClient(base_url="http://localhost:2785/", api_key="k", transport=backend.as_transport())
        client.sessions.list()
    ...
    def test_404_maps_to_not_found_error(self):
        ...
        with pytest.raises(ZapturaNotFoundError):
    ...
    def test_maps_503_to_service_unavailable(self):
        ...
        with pytest.raises(ZapturaServiceUnavailableError):
