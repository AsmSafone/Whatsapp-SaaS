...
public final class ClientConfig {
    ...
    final HttpTransport transport; // nullable → ZapturaClient supplies DefaultHttpTransport

    private ClientConfig(Builder b) {
        String url = b.baseUrl == null ? null : b.baseUrl.strip();
        if (url == null || url.isEmpty()) {
            throw new IllegalArgumentException("ZapturaClient: baseUrl is required");
        }
        try {
            new URI(url);
        } catch (URISyntaxException e) {
            throw new IllegalArgumentException("ZapturaClient: baseUrl is not a valid URL: " + url);
        }
        String key = b.apiKey == null ? null : b.apiKey.strip();
        if (key == null || key.isEmpty()) {
            throw new IllegalArgumentException("ZapturaClient: apiKey is required");
        }
        if (hasControlChar(key)) {
            throw new IllegalArgumentException(
                "ZapturaClient: apiKey contains illegal characters (whitespace or control) — check for a stray newline");
        }
        if (b.timeout != null && (b.timeout.isZero() || b.timeout.isNegative())) {
            throw new IllegalArgumentException("ZapturaClient: timeout must be positive");
        }
        ...
        try {
            ...
            if ("http".equalsIgnoreCase(scheme) && host != null) {
                ...
                if (!h.equals("localhost") && !h.equals("127.0.0.1") && !h.equals("::1")) {
                    System.err.println(
                        "[Zaptura SDK] WARNING: baseUrl uses an insecure http:// URL (host: " + host + "). "
    ...
    public static final class Builder {
        ...
        /** Base URL of the Zaptura API, e.g. {@code http://localhost:2785}. */
        ...
        /** Injectable transport; defaults to {@link com.asmsafone.zaptura.http.DefaultHttpTransport}. */
