 ...
 * subclass.
 */
public class ZapturaApiError extends ZapturaError {
    ...
    public ZapturaApiError(String message, int status, Object body, String errorKind) {
        super(message);
        this.status = status;
        this.body = body;
     ...
     * surfaces as a generic {@link ZapturaApiError} whose message states the
     * redirect was not followed (the API key is never re-sent to a redirect
     * target).
     */
    public static ZapturaApiError fromResponse(int status, String statusText, String rawBody, String context) {
        ...
        String message = "Zaptura API " + status + reason + " — " + context + tail;
    ...
    public static ZapturaApiError classify(int status, String message, Object body, String errorKind) {
        return switch (status) {
            case 401 -> new ZapturaAuthError(message, status, body, errorKind);
            case 403 -> new ZapturaForbiddenError(message, status, body, errorKind);
            case 404 -> new ZapturaNotFoundError(message, status, body, errorKind);
            case 409 -> new ZapturaConflictError(message, status, body, errorKind);
            case 429 -> new ZapturaRateLimitError(message, status, body, errorKind);
            case 501 -> new ZapturaNotImplementedError(message, status, body, errorKind);
            case 503 -> new ZapturaServiceUnavailableError(message, status, body, errorKind);
            default -> new ZapturaApiError(message, status, body, errorKind);
        };
    }
}
