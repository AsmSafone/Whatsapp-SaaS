...
import com.asmsafone.zaptura.errors.ZapturaApiError;
import com.asmsafone.zaptura.errors.ZapturaError;
import com.asmsafone.zaptura.http.BinaryResponse;
import com.asmsafone.zaptura.http.DefaultHttpTransport;
import com.asmsafone.zaptura.http.Http;
import com.asmsafone.zaptura.http.HttpMethod;
import com.asmsafone.zaptura.model.MuteChatRequest;
import com.asmsafone.zaptura.model.UpdateSessionConfigRequest;
import com.asmsafone.zaptura.model.UpdateSessionConfigRequestSerializer;
import com.asmsafone.zaptura.model.UpdateSessionProxyRequest;
import com.asmsafone.zaptura.http.HttpRequestData;
...
import com.asmsafone.zaptura.resources.ChannelsResource;
import com.asmsafone.zaptura.resources.ChatsResource;
import com.asmsafone.zaptura.resources.ContactsResource;
import com.asmsafone.zaptura.resources.GroupsResource;
import com.asmsafone.zaptura.resources.HealthResource;
import com.asmsafone.zaptura.resources.LabelsResource;
import com.asmsafone.zaptura.resources.MessagesResource;
import com.asmsafone.zaptura.resources.ProfileResource;
import com.asmsafone.zaptura.resources.SearchResource;
import com.asmsafone.zaptura.resources.SessionsResource;
import com.asmsafone.zaptura.resources.StatusResource;
import com.asmsafone.zaptura.resources.TemplatesResource;
import com.asmsafone.zaptura.resources.WebhooksResource;
...
/**
 * The single entry point to the Zaptura SDK. Holds configuration and exposes
 ...
 * }</pre>
 */
public final class ZapturaClient {
    ...
    public ZapturaClient(String baseUrl, String apiKey) {
     ...
     * be JSON; a non-JSON body surfaces as a tidy {@link ZapturaError}, never a raw
     * Gson exception.
     */
    ...
    public <T> T request(HttpMethod method, String path, Object query, Object body, Class<T> type) {
        ...
        try {
            ...
            throw new ZapturaError("Non-JSON response — " + method + " " + path);
        }
    }
    ...
    private HttpResponseData execute(HttpMethod method, String path, Object query, Object body) {
        ...
        try {
            // A timeout surfaces as ZapturaTimeoutError (unchecked) and propagates.
            res = transport.send(reqData);
        } catch (IOException e) {
            throw new ZapturaError("Transport error — " + method + " " + path + ": " + e.getMessage());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ZapturaError("Transport interrupted — " + method + " " + path);
        } catch (IllegalArgumentException e) {
            // e.g. a JDK-restricted header name in defaultHeaders, or a URI-illegal char — keep it
            // inside the ZapturaError contract instead of leaking a raw JDK exception to the caller.
            throw new ZapturaError("Invalid request — " + method + " " + path + ": " + e.getMessage());
        }
        if (res.status() < 200 || res.status() >= 300) {
            throw ZapturaApiError.fromResponse(res.status(), "", utf8(res.body()), method + " " + path);
        }
        return res;
    }
}
