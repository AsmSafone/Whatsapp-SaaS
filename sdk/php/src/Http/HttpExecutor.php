...
namespace Zaptura\Http;

use GuzzleHttp\ClientInterface;
use GuzzleHttp\Exception\ConnectException;
use Zaptura\Exceptions\ZapturaApiException;
use Zaptura\Exceptions\ZapturaTimeoutException;
use Psr\Http\Message\ResponseInterface;

/**
 * Injectable HTTP transport for the Zaptura SDK.
 *
...
class HttpExecutor
{
     ...
     *
     * @throws ZapturaApiException  On any non-2xx response (typed subclass).
     * @throws ZapturaTimeoutException On timeout.
     */
    ...
    private function send(string $method, string $path, array $query, $body): ResponseInterface
    {
        ...
        try {
            ...
            if ($isTimeout) {
                throw new ZapturaTimeoutException($this->timeout);
            }
            throw $e;
        }
    ...
    private function buildApiException(ResponseInterface $response, string $method, string $path): ZapturaApiException
    {
        $status = $response->getStatusCode();
        $text = (string) $response->getBody();
        $data = null;
        ...
        $message = "Zaptura API {$status} {$reason} — {$method} {$path}: {$messageText}";

        return ZapturaApiException::classify($status, $message, $data, $envelope['error'] ?? null);
    }
}
