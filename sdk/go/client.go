// Package zaptura is the official Go client for the Zaptura WhatsApp API Gateway.
//
...
// Client is the entry point to the Zaptura API. Construct it with New. It is safe
...
func New(baseURL, apiKey string, opts ...Option) (*Client, error) {
	if strings.TrimSpace(baseURL) == "" {
		return nil, errors.New("zaptura: baseURL is required")
	}
	if strings.TrimSpace(apiKey) == "" {
		return nil, errors.New("zaptura: apiKey is required")
	}
...
const insecureWarning = "zaptura: baseURL uses insecure http:// — the API key is sent in cleartext; use https:// in production"
...
func (c *Client) do(ctx context.Context, method, path string, query url.Values, body, out any) error {
	...
	if err := json.Unmarshal(data, out); err != nil {
		...
		return fmt.Errorf("zaptura: decoding response: %w", err)
	}
	return nil
}
...
func (c *Client) doRaw(ctx context.Context, method, path string, query url.Values, body any) ([]byte, string, error) {
	...
	if body != nil {
		...
		if err != nil {
			return nil, "", fmt.Errorf("zaptura: encoding request body: %w", err)
		}
		bodyBytes = b
	...
	if err != nil {
		return nil, "", fmt.Errorf("zaptura: building request: %w", err)
	}
	if body != nil {
	...
	if err != nil {
		...
		return nil, "", fmt.Errorf("zaptura: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()

	data, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, "", fmt.Errorf("zaptura: reading response body: %w", err)
	}
