...
//	    zaptura.WithMiddleware(tracingMiddleware, metricsMiddleware),
//	)
...
func loggingMiddleware(log Logger) Middleware {
	return func(next http.RoundTripper) http.RoundTripper {
		return RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
			...
			if err != nil {
				log.Log(req.Context(), LevelError, "zaptura request failed",
					"method", req.Method, "url", req.URL.String(),
					"duration_ms", dur.Milliseconds(), "error", err.Error())
				return resp, err
			}
			log.Log(req.Context(), LevelDebug, "zaptura request",
				"method", req.Method, "url", req.URL.String(),
