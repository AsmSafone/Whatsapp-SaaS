/**
 * Typed error hierarchy for the Zaptura SDK.
 *
 * The Zaptura API returns NestJS-default errors of the shape:
 *   `{ statusCode: number, message: string | string[], error?: string }`
 * `error` is absent whenever the exception carried no explicit message, so it is never required to
 * recognise the envelope. This module maps that to a typed, ergonomic error tree so callers can
 * `instanceof`-check or branch on `.status`.
 *
 * @packageDocumentation
 */

/** Base class for every error thrown by the SDK. */
export class ZapturaError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'ZapturaError';
  }
}

/**
 * Thrown when the API responds with a non-2xx status. Carries the HTTP status
 * code and the parsed error body (or the raw text if the body was not JSON).
 *
 * Use the static {@link ZapturaApiError.fromResponse} factory in most cases.
 */
export class ZapturaApiError extends ZapturaError {
  /** HTTP status code (e.g. 400, 404, 409, 429, 501). */
  readonly status: number;
  /** Parsed JSON body if available, otherwise the raw response text. */
  readonly body: unknown;
  /** Value of the `error` field in the NestJS error envelope, if present. */
  readonly errorKind?: string;

  constructor(message: string, status: number, body: unknown, errorKind?: string) {
    super(message);
    this.name = 'ZapturaApiError';
    this.status = status;
    this.body = body;
    this.errorKind = errorKind;
  }

  /** Build an {@link ZapturaApiError} from a fetch Response, awaiting its body. */
  static async fromResponse(res: Response, context: string): Promise<ZapturaApiError> {
    // An opaque unfollowed redirect (we set `redirect: 'manual'`) surfaces as status 0, not a 3xx.
    // Give it a clear message instead of "Zaptura API 0": the redirect was deliberately not followed
    // so the API key is never re-sent to the redirect target.
    if (res.status === 0) {
      return new ZapturaApiError(
        `Unexpected redirect (not followed; the API key is never re-sent to a redirect target) — ${context}`,
        0,
        undefined,
      );
    }
    let body: unknown = undefined;
    const text = await res.text().catch(() => '');
    if (text) {
      try {
        body = JSON.parse(text);
      } catch {
        body = text;
      }
    }
    const env = isNestEnvelope(body) ? body : undefined;
    const messageText = describeMessage(env?.message ?? body ?? res.statusText);
    const message = `Zaptura API ${res.status} ${res.statusText} — ${context}: ${messageText}`;
    return new ZapturaApiError(message, res.status, body, env?.error);
  }
}

/** 401 Unauthorized — missing or invalid API key. */
export class ZapturaAuthError extends ZapturaApiError {}
/** 403 Forbidden — the API key's role is insufficient for this endpoint. */
export class ZapturaForbiddenError extends ZapturaApiError {}
/** 404 Not Found. */
export class ZapturaNotFoundError extends ZapturaApiError {}
/** 409 Conflict — typically an {@link EngineNotReadyError} from the backend. */
export class ZapturaConflictError extends ZapturaApiError {}
/** 429 Too Many Requests — rate limited. */
export class ZapturaRateLimitError extends ZapturaApiError {}
/** 501 Not Implemented — the active engine does not support this operation. */
export class ZapturaNotImplementedError extends ZapturaApiError {}

/**
 * 503 Service Unavailable — a transport failure, not a refusal. The gateway answers this when the
 * engine did not confirm the operation in time: WhatsApp never replied, the socket was down, or the
 * request budget ran out. **Retryable**, unlike every other typed error here.
 *
 * Not every 503 is safe to repeat blindly: the non-idempotent sends (group create, channel create,
 * media send) are deliberately left unbounded by the gateway so a slow WhatsApp reply never answers
 * one, and in a multi-node deployment a forwarded request answers 503 only when the owner node was
 * never reached. A forward that fails after the request was sent answers 502 or 504 instead (a plain
 * `ZapturaApiError`): the owner may already have carried it out, so do not repeat a non-idempotent
 * send on those unchecked.
 */
export class ZapturaServiceUnavailableError extends ZapturaApiError {}

/** Thrown when a request exceeds the configured timeout. */
export class ZapturaTimeoutError extends ZapturaError {
  constructor(timeoutMs: number) {
    super(`Request timed out after ${timeoutMs}ms`);
    this.name = 'ZapturaTimeoutError';
  }
}

/**
 * Construct the most specific {@link ZapturaApiError} subclass for a status code.
 * Falls back to the generic {@link ZapturaApiError} for unmapped statuses.
 */
export function classifyApiError(status: number, message: string, body: unknown, errorKind?: string): ZapturaApiError {
  switch (status) {
    case 401:
      return new ZapturaAuthError(message, status, body, errorKind);
    case 403:
      return new ZapturaForbiddenError(message, status, body, errorKind);
    case 404:
      return new ZapturaNotFoundError(message, status, body, errorKind);
    case 409:
      return new ZapturaConflictError(message, status, body, errorKind);
    case 429:
      return new ZapturaRateLimitError(message, status, body, errorKind);
    case 501:
      return new ZapturaNotImplementedError(message, status, body, errorKind);
    case 503:
      return new ZapturaServiceUnavailableError(message, status, body, errorKind);
    default:
      return new ZapturaApiError(message, status, body, errorKind);
  }
}

/**
 * Narrow the NestJS error envelope shape: `{ statusCode, message, error }`.
 *
 * `error` is optional. NestJS omits it whenever the exception was constructed without an explicit
 * message — which is what the global ValidationPipe does under `disableErrorMessages`, the default
 * when `NODE_ENV=production` and `VALIDATION_ERROR_DETAIL` is unset. Every rejected request in a
 * stock production deployment therefore answers `{ statusCode, message }` and nothing else.
 */
interface NestErrorEnvelope {
  statusCode: number;
  message: string | string[];
  error?: string;
}

function isNestEnvelope(body: unknown): body is NestErrorEnvelope {
  return typeof body === 'object' && body !== null && 'statusCode' in body && 'message' in body;
}

function describeMessage(message: string | string[] | unknown): string {
  if (Array.isArray(message)) return message.join(', ');
  if (typeof message === 'string') return message;
  return String(message);
}
