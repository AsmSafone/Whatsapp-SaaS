...
import { classifyApiError, ZapturaApiError, ZapturaTimeoutError } from './errors.js';
...
/**
 * Perform a single request against the Zaptura API and return the parsed JSON
 * body (or `null` for 204). Throws a typed {@link ZapturaApiError} subclass on
 * non-2xx, or {@link ZapturaTimeoutError} on timeout.
 */
...
async function send<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const apiError = await ZapturaApiError.fromResponse(res, context);
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      throw new ZapturaTimeoutError(timeoutMs);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
export function warnIfInsecureHttpUrl(url: string, label = 'baseUrl'): void {
  try {
    ...
    if (parsed.protocol === 'http:' && !LOCALHOST_HOSTS.has(parsed.hostname.toLowerCase())) {
      console.warn(
        `[Zaptura SDK] ${label} uses an insecure http:// URL (host: ${parsed.hostname}). ` +
