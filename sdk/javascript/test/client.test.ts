...
function client(transport: MockTransport): ZapturaClient {
...
describe('ZapturaClient', () => {
  it('requires baseUrl and apiKey', () => {
    expect(() => new ZapturaClient({ baseUrl: '', apiKey: 'x' })).toThrow();
    expect(() => new ZapturaClient({ baseUrl: 'http://x', apiKey: '' })).toThrow();
  });
  ...
  it('strips a trailing slash from baseUrl', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', { body: [] });
    const c = new ZapturaClient({ baseUrl: 'http://localhost:2785/', apiKey: 'k', fetch: t.asFetch() });
  ...
  it('does not auto-follow redirects (passes redirect: manual to fetch)', async () => {
    ...
    const c = new ZapturaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: recordingFetch });
  ...
  it('treats an unfollowed redirect (3xx) as an error', async () => {
    ...
    const c = new ZapturaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: redirectingFetch });
    await expect(c.sessions.list()).rejects.toThrow();
  });

  it('surfaces a real opaque unfollowed redirect (status 0) as a clear ZapturaApiError', async () => {
    ...
    const c = new ZapturaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: opaqueRedirectFetch });
    const err = await c.sessions.list().catch((e: unknown) => e);
    expect(err).toBeInstanceOf(ZapturaApiError);
    expect((err as ZapturaApiError).status).toBe(0);
    expect((err as ZapturaApiError).message).toMatch(/redirect/i);
  });
  ...
  it('maps a 404 to ZapturaNotFoundError with parsed body', async () => {
    const t = new MockTransport().on('GET', '/api/sessions/missing', {
      status: 404,
      body: { statusCode: 404, message: 'Session not found', error: 'Not Found' },
    });
    await expect(client(t).sessions.get('missing')).rejects.toBeInstanceOf(ZapturaNotFoundError);
    await expect(client(t).sessions.get('missing')).rejects.toMatchObject({ status: 404 });
  });

  it('maps a 503 to ZapturaServiceUnavailableError', async () => {
  ...
  it('ZapturaApiError.fromResponse parses the NestJS envelope', async () => {
  ...
  it('parses the envelope a production gateway actually sends, with no `error` key', async () => {
    ...
    expect(err).toBeInstanceOf(ZapturaApiError);
    expect((err as ZapturaApiError).message).toContain('Bad Request');
    expect((err as ZapturaApiError).message).not.toContain('[object Object]');
    expect((err as ZapturaApiError).status).toBe(400);
    // No `error` key was sent, so there is no kind to report — undefined, not a stringified object.
    expect((err as ZapturaApiError).errorKind).toBeUndefined();
  });
  ...
  it('still reads `error` as the kind when the gateway sends one', async () => {
    ...
    expect((err as ZapturaApiError).errorKind).toBe('Bad Request');
    expect((err as ZapturaApiError).message).toContain('name must be a string');
  });

  it('maps each status code to its typed error subclass', async () => {
    const cases: Array<[number, new (...a: never[]) => ZapturaApiError]> = [
  ...
  it('falls back to the generic ZapturaApiError (with .status) for an unmapped status', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', {
      status: 418,
      body: { statusCode: 418, message: 'teapot', error: 'Teapot' },
    });
    await expect(client(t).sessions.list()).rejects.toMatchObject({ status: 418 });
    await expect(client(t).sessions.list()).rejects.toBeInstanceOf(ZapturaApiError);
  });

  it('throws ZapturaTimeoutError when the request aborts', async () => {
    const abortingFetch: FetchLike = async () => {
      const e = new Error('aborted');
      e.name = 'AbortError';
      throw e;
    };
    const c = new ZapturaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: abortingFetch });
    await expect(c.sessions.list()).rejects.toBeInstanceOf(ZapturaTimeoutError);
  });
  ...
  it('keeps the timeout armed while reading a stalled response body', async () => {
    ...
    const c = new ZapturaClient({ baseUrl: 'http://x', apiKey: 'k', timeoutMs: 5, fetch: stalledBodyFetch });
